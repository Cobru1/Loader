...
if success and NotificationLib then
    NotificationLib:Show("￢ﾚﾡ Loader", "Library loaded!", 3)
    ...
    if findInTable(placeId, FIND_MARKERS_PLACEIDS) then
        NotificationLib:Show("￰ﾟﾎﾯ Loader", "Game detected: Find the Markers", 5)
    ...
    elseif placeId == RACING_SIM_PLACEID then
        NotificationLib:Show("￰ﾟﾏﾎ￯ﾸﾏ Loader", "Game detected: Racing Simulator", 5)
        ...
        NotificationLib:Show("￢ﾝﾌ Loader", "Game not supported: "..gameName, 6)
    end
else
